...
const cachePath = exports.cachePath = _path2.default.join(_pkgDir2.default.sync(process.cwd()), 'cypress', '.snapshot-report');
...
function matchImageSnapshotResult() {
  return () => {
    ...
    if (!pass && !added && !updated && _fsExtra2.default.existsSync(cachePath)) {
      const cache = JSON.parse(_fsExtra2.default.readFileSync(cachePath, 'utf8'));
...
function matchImageSnapshotPlugin({ path: screenshotPath }) {
  ...
  const snapshotsDir = customSnapshotsDir ? _path2.default.join(process.cwd(), customSnapshotsDir, relativePath) : _path2.default.join(screenshotsFolder, '..', 'output', relativePath);
  ...
  const diffDir = customDiffDir ? _path2.default.join(process.cwd(), customDiffDir, relativePath) : _path2.default.join(snapshotsDir, '_diff_output_screen_');
  ...
  snapshotResult = (0, _diffSnapshot.diffImageToSnapshot)(_extends({
    snapshotsDir,
    diffDir,
    receivedImageBuffer,
    snapshotIdentifier,
